...
public class BioCircuitFancyConfigurator extends CircuitFancyConfigurator {
    ...
    public Widget createConfigurator() {
        ...
        group.addWidget((new SlotWidget(this.circuitSlot, 0, (group.getSize().width - 18) / 2, 20, false, false)).setBackground(new GuiTextureGroup(CBGuiTextures.SLOT, GuiTextures.INT_CIRCUIT_OVERLAY)));
        ...
        for(x = 0; x <= 2; ++x) {
            for(finalIdx = 0; finalIdx <= 8; ++finalIdx) {
                int finalIdx1 = idx;
                group.addWidget(new ButtonWidget(5 + 18 * finalIdx, 48 + 18 * x, 18, 18, new GuiTextureGroup(new IGuiTexture[]{CBGuiTextures.SLOT, (new ItemStackTexture(new ItemStack[]{IntCircuitBehaviour.stack(idx)})).scale(0.8888889F)}), (clickData) -> {
                    if (!clickData.isRemote) {
        ...
        for(x = 0; x <= 5; ++x) {
            ...
            group.addWidget(new ButtonWidget(5 + 18 * x, 102, 18, 18, new GuiTextureGroup(new IGuiTexture[]{CBGuiTextures.SLOT, (new ItemStackTexture(new ItemStack[]{IntCircuitBehaviour.stack(finalIdx)})).scale(0.8888889F)}), (clickData) -> {
                if (!clickData.isRemote) {
